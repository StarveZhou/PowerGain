...
/**
 * Created by StarveZhou on 2018/1/15.
 */
...
class UndistortionFrameRender extends FrameRender {
    ...
    private static final String TAG = "SCV::CameraInfo";
    @Override
    public Mat render(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        String str = "R/C : ";
        str += mCalibrator.getCameraMatrix().rows() + ";" + mCalibrator.getCameraMatrix().cols() + "\n";
        for (int i=0; i<mCalibrator.getCameraMatrix().rows(); i++){
            for (int j=0; j<mCalibrator.getCameraMatrix().cols(); j++){
                str += mCalibrator.getCameraMatrix().get(i, j)[0] + "  ";
            }
            str += "\n";
        }
        Log.w(TAG, "cameraMatrix : " + str);
